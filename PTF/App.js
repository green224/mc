...
var HEAPF64 = new Float64Array(buffer);
  ...
  HEAP32[DYNAMICTOP_PTR>>2] = 1863568;
...
// {{PRE_LIBRARY}}
  ...
  function _API_get_Angle() {return AppGameData.get_Angle();}

  function _API_get_ClearFlag(idx) {return AppGameData.get_ClearFlag(idx);}

  function _API_get_Position(axisIdx) {return AppGameData.get_Position(axisIdx);}

  function _API_get_StgIdx() {return AppGameData.get_StgIdx();}

  function _API_saveAll() {AppGameData.saveAll();}

  function _API_set_Angle(value) {AppGameData.set_Angle(value);}

  function _API_set_ClearFlag(idx, value) {AppGameData.set_ClearFlag(idx, value);}

  function _API_set_Position(axisIdx, value) {AppGameData.set_Position(axisIdx, value);}

  function _API_set_StgIdx(value) {AppGameData.set_StgIdx(value);}
  ...
  function _API_setup() {
  		...
  		let uint8toBase64 = (uint8Array) => {
  			return btoa(String.fromCharCode.apply(null, uint8Array));
  		};
  
  		// BASE64をUint8Arrayに変換する
  		let base64toUint8 = (base64) => {
  		...
  		// UInt8二つをパックしたものと、Floatとの相互変換処理。
  		// floatの範囲は -327.68 ~ 327.67
  		let float2Uint8Pack = (value) => {
  			value = Math.max(0, Math.floor(value*100 + 128*256));
  			return [ Math.min(Math.floor(value/256),255), value%256, ];
  		};
  		let uint8Pack2Float = (value0, value1) => {
  			let a = value0*256 + value1;
  			return (a - 128*256) / 100;
  		};
  
  		// セーブデータを読み込み/書き込み
  		let loadSaveData = () => {
  			let sdBase64 = localStorage.getItem("PTF_SD");
  			if (sdBase64==null) {
  				console.log("[PTF] There is no savedata");
  				return;
  			}
  
  			let sdU8Ary = base64toUint8(sdBase64);
  			let version = sdU8Ary[0];
  			if (version!=AppGameData.version[0]) {
  				console.log("[PTF] This savedata is old version:"+version);
  				return;
  			}
  			...
  			let loadOne = len => {
  				i += len;
  				return sdU8Ary.slice(i-len, i);
  			};
  			AppGameData.curStgIdx = loadOne(1);
  			AppGameData.clearFlags = loadOne(16);
  			AppGameData.position = loadOne(6);
  			AppGameData.angle = loadOne(2);
  		};
  		let saveSaveData = () => {
  			...
  			let sdU8Ary = null;
  			{
  				let subAryLst = [
  					AppGameData.version,
  					AppGameData.curStgIdx,
  					AppGameData.clearFlags,
  					AppGameData.position,
  					AppGameData.angle,
  				];
  				let ttlLen = 0;
  				for (let i=0; i<subAryLst.length; ++i) ttlLen += subAryLst[i].length;
  				sdU8Ary = new Uint8Array(ttlLen);
  				let ofs = 0;
  				for (let i=0; i<subAryLst.length; ++i) {
  					sdU8Ary.set(subAryLst[i], ofs);
  					ofs += subAryLst[i].length;
  				}
  			}
  
  			let sdBase64 = uint8toBase64(sdU8Ary);
  
  			localStorage.setItem("PTF_SD", sdBase64);
  			console.log("[PTF] saved");
  		};
  		...
  		// ゲームデータを生成
  		AppGameData = {
  			version: new Uint8Array(1),			// アプリバージョン
  			curStgIdx: new Uint8Array(1),		// 現在のステージIndex
  			clearFlags: new Uint8Array(16),		// クリア状況
  			position: new Uint8Array(6),		// 現在位置情報
  			angle: new Uint8Array(2),			// 向き
  
  			// ----------------- メンバ関数の定義 ---------------------
  
  			// 現在のステージIndexを読み込み/書き込み
  			get_StgIdx: () => AppGameData.curStgIdx[0],
  			set_StgIdx: (value) => AppGameData.curStgIdx[0] = value,
  
  			// 指定のIndexのマップがクリアしているか否かを読み込み/書き込み
  			get_ClearFlag: idx => {
  				let cfIdx = Math.floor(idx / 8);
  				let a = AppGameData.clearFlags[cfIdx];
  				let mask = 1 << (idx % 8);
  				return (a & mask) != 0;
  			},
  			set_ClearFlag: (idx, value) => {
  				let cfIdx = Math.floor(idx / 8);
  				let a = AppGameData.clearFlags[cfIdx];
  				let mask = 1 << (idx % 8);
  				AppGameData.clearFlags[cfIdx] = (a & ~mask) | (value ? mask : 0);
  			},
  
  			// 現在位置情報を読み込み/書き込み
  			get_Position: axisIdx => {
  				return uint8Pack2Float(
  					AppGameData.position[axisIdx*2 + 0],
  					AppGameData.position[axisIdx*2 + 1]
  				);
  			},
  			set_Position: (axisIdx, value) => {
  				let a = float2Uint8Pack(value);
  				AppGameData.position[axisIdx*2 + 0] = a[0];
  				AppGameData.position[axisIdx*2 + 1] = a[1];
  			},
  			...
  			set_Angle: value => {
  				let a = float2Uint8Pack(value);
  				AppGameData.angle[0] = a[0];
  				AppGameData.angle[1] = a[1];
  			},
  ...
  var _fetch_work_queue=1339264;function __emscripten_get_fetch_work_queue() {
...
dynCall_iiffiii = asm["dynCall_iiffiii"];
dynCall_viiffii = asm["dynCall_viiffii"];
